 ...
 product_ids = {p[0] for p in products}
    ...
    for w in waste:
        ...
        if sale_item_id is not None:
            ...
            if sale_item_id not in sale_item_ids:
...
def run_all_validations(stores, products, employees, shifts, sales, sale_items, waste):
 all_errors = []
    all_errors += check_required_columns_populated(stores, products, employees)
    all_errors += check_foreign_keys_resolve(sale_items, sales, waste, products, employees, stores)
    all_errors += check_no_negative_quantities(sale_items, waste)
    all_errors += check_no_negative_prices(products, sale_items, sales)
    all_errors += check_sale_times_within_store_hours(sales)
    all_errors += check_sales_employee_on_shift(sales, shifts)
